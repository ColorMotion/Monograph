...
\hspace{5cm}
\assinatura{Tiago Koji Castro Shibata \\(Student)}
\hspace{5cm}
...
%citation needed
where the layer is composed of a small kernel that finds the maximum value in a sub-matrix with the same dimensions as the kernel. This kernel is applied to the input matrix, according to striding (skipping step), similarly to the striding defined in the convolutional layer. Sub-sampling can also be done on the convolutional layer itself. For instance, Iizuka \textit{et al.} \cite{Iizuka2016} applies a stride of 2 in order to downsample.

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{CNN}
\caption{\acrshort{cnn} \textit{(from River Trail documentation \cite{river_trail})}}
\label{cnn_fig}
\end{figure}
...
The decoder has transposed convolutions to merge the information from multiple filters and rescale the image. Skip connections are also used to pass lower level details to the decoder layers.

\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{encoder_decoder}
\caption{Encoder-decoder architecture \cite{proposed_segmentation}}
\label{segm_prop}
\end{figure}
