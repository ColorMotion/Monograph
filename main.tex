...
\hspace{5cm}
\assinatura{Tiago Castro Koji Shibata \\(Orientada)}
\hspace{5cm}
\assinatura{Henrique Cassiano Souza Barros \\(Orientada)}
\hspace{5cm}
\assinatura{Victoria Akina Tanaka \\(Orientada)}
...
%TODO: diagrama de exemplo de CNN?
...
We implemented a similar architecture in Keras. For simplicity, we use a classical dense optical flow instead of a flow regression neural network. The authors of the original architecture did not specify their mask regression network structure; we used , %TODO
which worked well for us.

%TODO Explain train > validation
